...
const Slider = ({ title, description, price, image, isActive }) => {
  ...
  return (
    <div className={`absolute w-full h-full md:flex justify-between items-center px-12 transition-opacity duration-500 ${isActive ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      <div ref={contentRef} className="max-w-lg z-10">
        ...
        <p className="text-white/80 mb-8 hidden md:flex text-base">{description}</p>
